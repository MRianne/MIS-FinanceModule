...
function initializeDoughnutChart(sy, term){
  ...
  function(data){
    ...
    if(data["overall"]["pie"]["remaining"] != null)
      ...
      doughnutChart(data["overall"]["pie"], color["expense"],
                    data["overall"]["status"], "overallPie");

      doughnutChart(data["trasaction"]["pie"], color["transaction"],
...
function doughnutChart(data, colors, title, pie){
  ...
  $.each(data, function( index, value ) {
    if(value > 0){
      ...
      d["title"] = index;
      d["value"] = value;
      d["color"] = "rgb(" + colors["r"][0] + "," + colors["g"][0] + "," + colors["b"][0] + ")";
      dataCharts.push(d);
      colors["r"][0] = colors["r"][0] - colors["r"][1];
      colors["g"][0] = colors["g"][0] - colors["g"][1];
      colors["b"][0] = colors["b"][0] - colors["b"][1];
    }
  });
  doughnutChartSetting();
  $("#" + pie).drawDoughnutChart(title, dataCharts);
}

function doughnutChartSetting(){
  $.fn.drawDoughnutChart = function(title, data, options) {
    var $this = this,
      W = $this.width(),
      H = $this.height(),
      centerX = W/2,
